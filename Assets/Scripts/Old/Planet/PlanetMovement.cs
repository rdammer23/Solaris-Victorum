...
{
    class PlanetMovement : SimpleNetworkedMonoBehavior
    {
        private bool movePlanet = true;
        ...
        void Start()
        {
            if(IsServerOwner)
            {
                ...
                starPosition = transform.parent;
                orbitSpeed = defaultOrbitSpeed / Vector3.Distance(this.transform.position, starPosition.position);
        ...
        void Update()
        {
            if (IsServerOwner)
            {
                if (movePlanet)
                {
                    ...
                    this.transform.RotateAround(starPosition.position, new Vector3(0, 1, 0), orbitSpeed * Time.deltaTime);
                }
            }
        }
    }
