...
namespace Assets.Scripts.Player {
    class NetworkSetup: NetworkedMonoBehavior {
        ...
        private void GetPlayers(List<NetworkingPlayer> players) {
            BeardedManStudios.Network.Unity.MainThreadManager.Run(() => {
                ...
                foreach (KeyValuePair<ulong, SimpleNetworkedMonoBehavior> kv in networkedBehaviors) {
                    if (kv.Value is NetworkSetup) {
                        foreach (NetworkingPlayer player in players) {
                            if (kv.Value.OwnerId == player.NetworkId) {
                                ((NetworkSetup)kv.Value).nameMesh.text = !string.IsNullOrEmpty(player.Name) ? player.Name : "Client";
                                break;
                            }
                        }
                    }
