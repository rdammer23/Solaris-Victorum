...
namespace BeardedManStudios.Network
{
	public class Networking
	{
        ...
        //THIS IS MY OVERRIDE IN ORDER TO CHANGE THE INSTANCE NAME OF THE PREFAB
        private static void CallInstantiate(string obj, string instanceName, Vector3 position, Quaternion rotation, NetworkReceivers receivers, Action<SimpleNetworkedMonoBehavior> callback = null) {
            SimpleNetworkedMonoBehavior netBehavior;
            if (ValidateNetworkedObject(obj, out netBehavior)) {
                if (callback != null) {
                    instantiateCallbacks.Add(callbackCounter, callback);
                    NetworkingManager.Instantiate(receivers, obj, position, rotation, callbackCounter, instanceName);
                    ...
                    NetworkingManager.Instantiate(receivers, obj, position, rotation, 0, instanceName);
            }
        }
        ...
        /// <summary>
        /// THIS IS MY OVERRIDE IN ORDER TO CHANGE THE INSTANCE NAME OF THE PREFAB ON INSTANTIATE
        /// Instantiate an object on the network
        /// </summary>
        /// <param name="obj">Object to be instantiated by object name</param>
        /// <param name="position">Position of instantiated object</param>
        /// <param name="rotation">Rotation of instantiated object</param>
        /// <param name="receivers">Recipients will receive this instantiate call (Default: All)</param>
        public static void Instantiate(GameObject obj, string instanceName, Vector3 position, Quaternion rotation, NetworkReceivers receivers = NetworkReceivers.All, Action<SimpleNetworkedMonoBehavior> callback = null) {
            if (!NetworkingManager.IsOnline) {
                // JM: offline fixes
                SimpleNetworkedMonoBehavior snmb = (GameObject.Instantiate(obj, position, rotation) as GameObject).GetComponent<SimpleNetworkedMonoBehavior>();
                snmb.OfflineStart();
            ...
            if (NetworkingManager.Instance == null || !NetworkingManager.Instance.IsSetup) {
                ...
                CallInstantiate(obj.name, instanceName, position, rotation, receivers, callback);
