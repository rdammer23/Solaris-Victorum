...
namespace BeardedManStudios.Network
{
	...
	public class NetworkingManager : SimpleNetworkedMonoBehavior
	{
        ...
        /// <summary>
        /// THIS IS MY OVERRIDE IN ORDER TO CHANGE THE PREFAB INSTANCE NAME
        /// This is the main instantiate method that is proxyed through the Networking.Instantiate method
        /// </summary>
        /// <param name="receivers">The receivers that will get this instantiate command</param>
        /// <param name="obj">The name of the object that is to be instantiated</param>
        ...
        /// <param name="rotation">The rotation that the object will have when instantiated</param>
        /// <param name="callbackCounter">The id for the callback that is to be called when this object is instantiated</param>
        /// <param name="instanceName">The name the instance of the prefab should be called</param>
        public static void Instantiate(NetworkReceivers receivers, string obj, Vector3 position, Quaternion rotation, int callbackCounter, string instanceName) {
            ulong uniqueId = 0;
            if (!TryPullIdFromObject(obj, ref uniqueId)) {
                return;
            }
            ...
            if (Instance != null && Instance.OwningNetWorker != null) {
                Instance.RPC("NetworkInstantiateNewName", receivers, Instance.OwningNetWorker.Uniqueidentifier, Instance.OwningNetWorker.IsServer ? uniqueId : 0, obj, instanceName, position, rotation, callbackCounter);
            }
            else {
                setupActions.Add(() => {
                    Instance.RPC("NetworkInstantiateNewName", receivers, Instance.OwningNetWorker.Uniqueidentifier, Instance.OwningNetWorker.IsServer ? uniqueId : 0, obj, instanceName, position, rotation, callbackCounter);
        ...
        // THIS IS MY OVERRIDE IN ORDER TO CHANCE THE INSTANCE NAME OF THE PREFAB, MUST BE A STRING
        [BRPC]
        public void NetworkInstantiateNewName(ulong ownerId, ulong startNetworkId, string name, string instanceName, Vector3 position, Quaternion rotation, int callbackId = 0) {
            lock (networkedBehaviorsMutex) {
                if (networkedBehaviors.ContainsKey(startNetworkId))
                    return;
